...
%REVOLVE  gmsh.model.occ.revolve
...
%   the `numElements' vector is not empty, also extrude the mesh: the entries in
%   `numElements' give the number of elements in each layer. If the `height'
...
%   strictly smaller than 2*Pi. If `recombine' is set, recombine the mesh in the
%   layers.
%
%   Inputs:
...
%     outDimTags - Nx2 matrix of (dim,tag) pairs
    ...
    outDimTags = gmsh.internal.api.call('gmshModelOccRevolve', dimTags, x, y, z, ax, ay, az, angle, numElements, heights, recombine);
end
