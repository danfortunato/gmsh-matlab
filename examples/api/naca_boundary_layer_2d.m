...
% Top half of the NACA 0012 profile (x, y pairs).
...
pts = [];
for i = 0:l/2 - 1
    pts(end+1) = gmsh.model.occ.addPoint(naca(2*i + 1), naca(2*i + 2), 0, lc1); %#ok<AGROW>
end
for i = l/2 - 2:-1:0
    pts(end+1) = gmsh.model.occ.addPoint(naca(2*i + 1), -naca(2*i + 2), 0, lc1); %#ok<AGROW>
end
...
curv = [];
curv(end+1) = gmsh.model.occ.addSpline(pts);

if rounded
    c = gmsh.model.occ.addPoint(0.9985510, 0, 0, lc1);
    curv(end+1) = gmsh.model.occ.addCircleArc(pts(end), c, pts(1));
else
    pt = gmsh.model.occ.addPoint(1.0095, 0, 0, lc1);
    curv(end+1) = gmsh.model.occ.addLine(pts(end), pt); %#ok<NASGU>
    curv(end+1) = gmsh.model.occ.addLine(pt, pts(1));
end

dimtags = [ones(numel(curv), 1), curv(:)];
gmsh.model.occ.rotate(dimtags, 0.25, 0, 0, 0, 0, 1, incidence);
...
if by_extrusion
    ...
    extbl = gmsh.model.geo.extrudeBoundaryLayer( ...
        gmsh.model.getEntities(1), ones(1, N), d, true);
    top_curves = extbl(1:2:end, 2).';
    cl2 = gmsh.model.geo.addCurveLoop(top_curves);
    ...
    gmsh.model.occ.addPlaneSurface([cl2, cl]);
    gmsh.model.occ.synchronize();
